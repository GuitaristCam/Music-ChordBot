# This Makefile is for the Music::ChordBot extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Music/ChordBot.pm]
#     AUTHOR => [q[Johan Vromans <jv@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Music::ChordBot]
#     PL_FILES => {  }
#     PREREQ_PM => { JSON=>q[0], Test::More=>q[0], parent=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Music/ChordBot.pm]
#     clean => { FILES=>q[Music-ChordBot-*] }
...
INST_LIBDIR      = $(INST_LIB)/Music
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Music
...
DISTNAME = Music-ChordBot
DISTVNAME = Music-ChordBot-0.91
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Programmatically build songs for ChordBot'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Johan Vromans <jv@cpan.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Music-ChordBot' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: '\''0.91'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'x_serialization_backend: '\''CPAN::Meta::YAML version 0.018'\''' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Programmatically build songs for ChordBot",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Johan Vromans <jv@cpan.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Music-ChordBot",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "0.91",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Music-ChordBot" VERSION="0.91">' > Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Programmatically build songs for ChordBot</ABSTRACT>' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Johan Vromans &lt;jv@cpan.org&gt;</AUTHOR>' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="JSON::" />' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::More" />' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="parent::" />' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.38" />' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Music-ChordBot.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Music-ChordBot.ppd
